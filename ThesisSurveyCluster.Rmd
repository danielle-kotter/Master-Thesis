...
mydata <- scale(dataf[,c(2:6, 7:13)]) # standardize variables
mydata <- cbind(mydata, dataf[,c(14:50)]) 
```
...
fit <- kmeans(na.omit(mydata), 3, nstart = 25) #3 cluster solution
# get cluster means
...
```{r}
Results <- as.data.table(aggregate(na.omit(mydata[,19:25]), by=list(cluster=fit$cluster), mean), by = round)
...
```{r}
Results <- as.data.table(aggregate(na.omit(mydata[,6:9]), by=list(cluster=fit$cluster), mean), by = round)
...
```{r}
Results <- as.data.table(aggregate(na.omit(mydata[,35:40]), by=list(cluster=fit$cluster), mean), by = round)
...
```{r}
Results <- as.data.table(aggregate(na.omit(NumMydata[,1:5]), by=list(cluster=fit$cluster), mean), by = round)
...
```{r}
Results <- as.data.table(aggregate(na.omit(NumMydata[,6:9]), by=list(cluster=fit$cluster), mean), by = round)
...
```{r}
Results <- as.data.table(aggregate(na.omit(NumMydata[,10:12]), by=list(cluster=fit$cluster), mean), by = round)
