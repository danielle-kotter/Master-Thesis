...
knitr::opts_chunk$set(echo = FALSE, class.source = "watch-out", options(scipen=999), out.width = "60%", warning = FALSE, comment = "") 
source("www/init.R")
...
dani <- c( "#ffa500", "#DAF7A6", "#5F7992", "#69b3a2", "#ffd561", "#ee5c42", "#C8A2C8", "#5c3170", "#990000", "#C70039", "#34495E", "#909497")
...
ggplot(dataB, aes(x="",y = value, fill = group)) +
  geom_bar(width = 1, stat = "identity") +
  coord_polar("y", start = 0) +
  scale_fill_manual("Brand choice:", values = dani)+
  blank_theme  +
    theme(axis.text.x=element_blank(), 
          legend.title = element_text(face = "bold")) +
  geom_text(aes(x = 1.1, label = value),
...
NewTable <- as.data.table(dt_l[,table(as.factor(value))])

NewTable$V1 <- as.factor(NewTable$V1)
NewTable <- NewTable[, list(y=sum(N)),by=V1]
...
NewTable <- as.data.table(data.frame(Reasons = c("I am satisfied with the product", "Price", "Time-saving", "Convenience",  "I do not purchase coffee from the supermarket", "I do not have specialty stores near where I live", "Other"), Frequency = c(90, 71, 56, 53, 45, 16, 3) ))
...
NewTable <- as.data.table(data.frame(Reasons = c("No reason", "Better quality elsewhere", "Not enough variety", "Not wanting to support big cooperations", "It is not fresh", "Lack of sustainable options", "I don't buy from supermarkets", "Price"), Frequency = c(102, 96, 28, 22, 17, 8, 7, 2) ))
...
```{r}
NewTable <- as.data.table(data.frame(Reasons = c("Flavour profile", "Price", "Roast level", "Origin", "Arabica or Robusta", "Sustainability & Fair Trade"), Frequency = c(149, 89, 64, 38, 18, 16) ))
...
ggplot(dataB, aes(x="",y = value, fill = group)) +
  geom_bar(width = 1, stat = "identity") +
  coord_polar("y", start = 0) +
  scale_fill_manual("Importance:", values = dani)+
  blank_theme  +
    theme(axis.text.x=element_blank(), 
...
NewTable<- as.data.table(NSubscription_Not_Likely)
colnames(NewTable) <- c("Reason", "Frequency")
my_table(NewTable)
```
...
```{r}
NewTable <- as.data.table(data.frame(Reasons = c("I do not like being stuck with subscriptions", "I am happy with my coffee now", "The price", "I do not consume enough coffee at home", "The packaging that is required for delivery", "No reason", "I already have a subscription", "Other"), Frequency = c(111, 109, 56, 22, 15, 15, 10, 4) ))
...
```{r}
chisq.test(Subscription_Likely, App_Likely)
chisq.test(Subscription_Likely, App_Likely, simulate.p.value = TRUE, B = 500)
```

```{r}
chisq.test(Subscription_Likely, KnowledgeCoffee)
chisq.test(Subscription_Likely, KnowledgeCoffee, simulate.p.value = TRUE, B = 500)
```

```{r}
chisq.test(Subscription_Likely, AmountWeek)
chisq.test(Subscription_Likely, AmountWeek, simulate.p.value = TRUE, B = 500)
```

```{r}
chisq.test(Subscription_Likely, Frequency_Specialty)
chisq.test(Subscription_Likely, Frequency_Specialty, simulate.p.value = TRUE, B = 500)
```

```{r}
chisq.test(Subscription_Likely, BrandChange)
chisq.test(Subscription_Likely, BrandChange, simulate.p.value = TRUE, B = 500)
```

```{r}
chisq.test(Subscription_Likely, PurchaseLocation)
chisq.test(Subscription_Likely, PurchaseLocation, simulate.p.value = TRUE, B = 500)
```
...
```{r}
 ggplot(data, aes(y = AmountWeek, x = MoneyCoffee)) +
  ...
  geom_smooth(method="lm", fill = NA, color="#69b3a2", fullrange=TRUE, formula = y ~ x) +
   theme(plot.title = element_text(hjust = 0.5)) +
        labs(title = "Amount of coffee consumed and the amount of money spend",
         y = "Coffee cups consumed per week",
...
```{r}
 ggplot(data, aes(y = AmountWeek, x = MoneyGroceries)) +
  ...
  geom_smooth(method="lm", fill = NA, color="#69b3a2", fullrange=TRUE, formula = y ~ x) +
   theme(plot.title = element_text(hjust = 0.5)) +
        labs(title = "Amount of coffee consumed by the amount of money spend on groceries",
         y = "Coffee cups consumed per week",
...
```{r}
model1 <- (lm(Subscription_Likely ~ KnowledgeCoffee + Purchase_Fairtrade))
model2 <- (lm(Subscription_Likely ~ KnowledgeCoffee + AmountWeek + MoneyCoffee))
